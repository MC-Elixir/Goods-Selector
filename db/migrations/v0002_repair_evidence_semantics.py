...
def _repair_query_attempts(connection: Connection) -> None:
    ...
    connection.exec_driver_sql(
        ...
        SELECT
            '0002_repair_evidence_semantics', 'query_attempts', id, 'result_count',
            CAST(result_count AS TEXT),
            CASE
                WHEN status = 'completed' THEN 'legacy_default_or_explicit_zero_unknown'
        ...
        UNION ALL
        SELECT
            '0002_repair_evidence_semantics', 'query_attempts', id, 'relevant_count',
            CAST(relevant_count AS TEXT),
            CASE
                WHEN status = 'completed' THEN 'legacy_default_or_explicit_zero_unknown'
                ELSE 'legacy_status_requires_null_metrics'
            END
        FROM query_attempts
        WHERE relevant_count IS NOT NULL"""
    )
    connection.exec_driver_sql(
        ...
        SELECT
            ...
            CASE WHEN status = 'completed' THEN result_count ELSE NULL END,
            CASE WHEN status = 'completed' THEN relevant_count ELSE NULL END,
            retry_of, status, artifact_ref, created_at
        FROM query_attempts"""
    )
...
def _repair_match_evidence(connection: Connection) -> None:
    ...
    connection.exec_driver_sql(
        ...
        SELECT
            ...
            CAST(overall_confidence AS TEXT), 'legacy_default_or_explicit_zero_unknown'
        FROM match_evidence
    ...
    connection.exec_driver_sql(
        ...
        SELECT
            ...
            CASE WHEN overall_confidence = 0 THEN NULL ELSE overall_confidence END,
            evidence_json, created_at
        FROM match_evidence"""
    )
